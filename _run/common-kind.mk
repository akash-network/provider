...
# is configured by engineered/setup-kind. `kind-control-plane` is the docker
...
ifeq ($(KIND_NAME), single)
	echo "- op: replace\n  path: /spec/template/spec/containers/0/image\n  value: $(AKASH_DOCKER_IMAGE)" > ./kustomize/akash-node/docker-image.yaml
	echo "- op: replace\n  path: /spec/template/spec/containers/0/image\n  value: $(DOCKER_IMAGE)" > ./kustomize/akash-provider/docker-image.yaml
endif
	echo "- op: replace\n  path: /spec/template/spec/containers/0/image\n  value: $(DOCKER_IMAGE)" > ./kustomize/akash-hostname-operator/docker-image.yaml
...
kind-cluster-create: $(KIND)
	...
	"$(AP_ROOT)/script/setup-kind.sh"

.PHONY: kind-install-operators
kind-install-operators:
...
kind-cluster-calico-create: $(KIND)
	...
	kubectl apply -f "$(INGRESS_CONFIG_PATH)"
	$(AP_ROOT)/script/setup-kind.sh calico-metrics
