...
func (c *client) Deploy(ctx context.Context, deployment ctypes.IDeployment) (err error) {
	...
	for svcIdx := range group.Services {
		...
		// Create ServiceAccount, Role, and RoleBinding if automountServiceAccountToken is enabled
		automountToken := workload.AutomountServiceAccountToken()
		if automountToken != nil && *automountToken {
			c.log.Info("creating ServiceAccount, Role, and RoleBinding for service", "lease", lid, "service", service.Name)
			...
			c.log.Debug("skipping ServiceAccount creation", "lease", lid, "service", service.Name, "automountToken", automountToken)
		}
