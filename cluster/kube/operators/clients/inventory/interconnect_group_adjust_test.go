...
// The group name becomes a pod label value verbatim; names Kubernetes would
// reject at admission must fail the bid, not the deployment — otherwise the
// tenant pays for a lease whose workloads can never be created.
func TestAdjust_InterconnectGroup_InvalidLabelRejected(t *testing.T) {
	inv := newInventory(interconnectGroupCluster(2))

	t.Run("slash in group name", func(t *testing.T) {
		res := interconnectGroupReservation(
			interconnectGroupResource(1, 1, "team/alpha"),
		)
		err := inv.Adjust(res)
		require.ErrorIs(t, err, ctypes.ErrInvalidInterconnectGroup)
	})

	t.Run("group name over 63 chars", func(t *testing.T) {
		res := interconnectGroupReservation(
			interconnectGroupResource(1, 1, strings.Repeat("a", 64)),
		)
		err := inv.Adjust(res)
		require.ErrorIs(t, err, ctypes.ErrInvalidInterconnectGroup)
	})

	t.Run("valid name still bids", func(t *testing.T) {
		res := interconnectGroupReservation(
			interconnectGroupResource(1, 1, "pair-0.a"),
		)
		require.NoError(t, inv.Adjust(res))
	})
}
