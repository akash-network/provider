...
func TestRolePolicyRules(t *testing.T) {
	...
	// TODO: enable events test in a different PR and test them separately.
...
func TestRBACResourceLabels(t *testing.T) {
	...
	serviceName := group.Services[0].Name

	t.Run("ServiceAccount has manifest-service label", func(t *testing.T) {
		saBuilder := BuildServiceAccount(workload)
		sa, err := saBuilder.Create()
		require.NoError(t, err)
		require.NotNil(t, sa)

		// Verify AkashManifestServiceLabelName is present
		require.Contains(t, sa.Labels, AkashManifestServiceLabelName)
		require.Equal(t, serviceName, sa.Labels[AkashManifestServiceLabelName])

		// Verify AkashManagedLabelName is present
		require.Contains(t, sa.Labels, AkashManagedLabelName)
		require.Equal(t, "true", sa.Labels[AkashManagedLabelName])
	})

	t.Run("Role has manifest-service label", func(t *testing.T) {
		roleBuilder := BuildRole(workload)
		role, err := roleBuilder.Create()
		require.NoError(t, err)
		require.NotNil(t, role)
		...
		// Verify AkashManifestServiceLabelName is present
		require.Contains(t, role.Labels, AkashManifestServiceLabelName)
		require.Equal(t, serviceName, role.Labels[AkashManifestServiceLabelName])

		// Verify AkashManagedLabelName is present
		require.Contains(t, role.Labels, AkashManagedLabelName)
		require.Equal(t, "true", role.Labels[AkashManagedLabelName])
	})

	t.Run("RoleBinding has manifest-service label", func(t *testing.T) {
		rbBuilder := BuildRoleBinding(workload)
		rb, err := rbBuilder.Create()
		require.NoError(t, err)
		require.NotNil(t, rb)

		// Verify AkashManifestServiceLabelName is present
		require.Contains(t, rb.Labels, AkashManifestServiceLabelName)
		require.Equal(t, serviceName, rb.Labels[AkashManifestServiceLabelName])

		// Verify AkashManagedLabelName is present
		require.Contains(t, rb.Labels, AkashManagedLabelName)
		require.Equal(t, "true", rb.Labels[AkashManagedLabelName])
	})
}
