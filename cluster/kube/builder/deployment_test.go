...
func TestDeploymentAutomountServiceAccountToken(t *testing.T) {
	...
	t.Run("respects automountServiceAccountToken: true", func(t *testing.T) {
		sdl, err := sdl.ReadFile("../../../testdata/sdl/automount-service-account-token.yaml")
		require.NoError(t, err)
		...
		// Test service with automountServiceAccountToken: true (index 0 = web)
		workload, err := NewWorkloadBuilder(log, settings, cdep, cmani, 0)
		require.NoError(t, err)
	...
	t.Run("respects automountServiceAccountToken: false", func(t *testing.T) {
		sdl, err := sdl.ReadFile("../../../testdata/sdl/automount-service-account-token.yaml")
		require.NoError(t, err)
		...
		// Test service with automountServiceAccountToken: false (index 1 = web2)
