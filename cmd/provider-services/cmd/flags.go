...
func addRunFlags(cmd *cobra.Command) error {
	...
	cmd.Flags().String(FlagCCKBSURL, "", "HTTPS Trustee KBS origin used by confidential guests")
	if err := viper.BindPFlag(FlagCCKBSURL, cmd.Flags().Lookup(FlagCCKBSURL)); err != nil {
		return err
	}

	cmd.Flags().String(FlagCCKBSCertFile, "", "Path to the public Trustee KBS certificate chain")
	if err := viper.BindPFlag(FlagCCKBSCertFile, cmd.Flags().Lookup(FlagCCKBSCertFile)); err != nil {
		return err
	}

	cmd.Flags().String(FlagCCImageSecurityPolicyURI, "", "Content-addressed kbs:/// image security policy URI for confidential guests")
	if err := viper.BindPFlag(FlagCCImageSecurityPolicyURI, cmd.Flags().Lookup(FlagCCImageSecurityPolicyURI)); err != nil {
		return err
	}

	cmd.Flags().String(FlagCCAgentPolicyFile, "", "Path to the measured Kata agent policy for confidential guests")
