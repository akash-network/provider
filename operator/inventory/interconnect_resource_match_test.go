...
func TestMatchInterconnectResourceName_MultiplePatternsFirstHitWins(t *testing.T) {
	// The matcher iterates configured patterns first, in order, so a
	// pattern with no matching resource is simply skipped and the next
	// pattern is tried. This test uses a single eligible resource; see
	// TestMatchInterconnectResourceName_FirstConfiguredPatternWins for
	// the ordering guarantee when multiple resources are eligible.
	got := matchInterconnectResourceName(
...
// A single pattern matching multiple advertised pools must resolve to a
// stable name: kubelet's ResourceList is a map, and a hash-seed-dependent
// flip between updateNodeInfo runs would strand running pods' allocations
// on the old name. Lexically-smallest match wins; operators who want a
// specific pool list a precise pattern first.
func TestMatchInterconnectResourceName_MultiMatchIsDeterministic(t *testing.T) {
	quantities := resList("cpu", "rdma/rdma_shared_device_ib", "rdma/rdma_shared_device_eth")

	for i := 0; i < 50; i++ {
		got := matchInterconnectResourceName(quantities, nil)
		require.Equal(t, corev1.ResourceName("rdma/rdma_shared_device_eth"), got,
			"lexically-smallest match must win on every call, independent of map iteration order")
	}

	// A precise pattern listed first overrides the lexical tie-break.
	got := matchInterconnectResourceName(quantities, []string{"rdma/rdma_shared_device_ib", "rdma/rdma_shared_device_*"})
	require.Equal(t, corev1.ResourceName("rdma/rdma_shared_device_ib"), got)
}
