...
type ProviderLeasedIPLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta2.ProviderLeasedIP, err error)
...
// List lists all ProviderLeasedIPs in the indexer.
func (s *providerLeasedIPLister) List(selector labels.Selector) (ret []*v2beta2.ProviderLeasedIP, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta2.ProviderLeasedIP))
	})
	return ret, err
}
...
type ProviderLeasedIPNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta2.ProviderLeasedIP, error)
...
// List lists all ProviderLeasedIPs in the indexer for a given namespace.
func (s providerLeasedIPNamespaceLister) List(selector labels.Selector) (ret []*v2beta2.ProviderLeasedIP, err error) {
...
// Get retrieves the ProviderLeasedIP from the indexer for a given namespace and name.
func (s providerLeasedIPNamespaceLister) Get(name string) (*v2beta2.ProviderLeasedIP, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta2.Resource("providerleasedip"), name)
	}
	return obj.(*v2beta2.ProviderLeasedIP), nil
}
