...
type ManifestLister interface {
	...
	List(selector labels.Selector) (ret []*v2beta2.Manifest, err error)
...
// List lists all Manifests in the indexer.
func (s *manifestLister) List(selector labels.Selector) (ret []*v2beta2.Manifest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2beta2.Manifest))
	})
	return ret, err
}
...
type ManifestNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2beta2.Manifest, error)
...
// List lists all Manifests in the indexer for a given namespace.
func (s manifestNamespaceLister) List(selector labels.Selector) (ret []*v2beta2.Manifest, err error) {
...
// Get retrieves the Manifest from the indexer for a given namespace and name.
func (s manifestNamespaceLister) Get(name string) (*v2beta2.Manifest, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2beta2.Resource("manifest"), name)
	}
	return obj.(*v2beta2.Manifest), nil
}
