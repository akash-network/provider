...
type InventoryRequestLister interface {
	...
	List(selector labels.Selector) (ret []*akashnetworkv2beta1.InventoryRequest, err error)
	// Get retrieves the InventoryRequest from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*akashnetworkv2beta1.InventoryRequest, error)
...
type inventoryRequestLister struct {
	listers.ResourceIndexer[*akashnetworkv2beta1.InventoryRequest]
}
...
func NewInventoryRequestLister(indexer cache.Indexer) InventoryRequestLister {
	return &inventoryRequestLister{listers.New[*akashnetworkv2beta1.InventoryRequest](indexer, akashnetworkv2beta1.Resource("inventoryrequest"))}
}
