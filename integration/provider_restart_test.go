...
// SetupSuite runs the provider as a real subprocess so a restart relaunches an actual
// process (fresh globals), not just a cancelled context in the same binary.
...
func (s *E2EProviderRestart) TestE2EProviderRestartKeepsWorkloads() {
	...
	// The recovery bug this guards against is non-convergent: one recovery only bumps
	// the Manifest's resource-version label, and a later recovery rolls the pods off it.
	// A single restart can pass against a buggy provider, so restart several times under
	// one watch: a buggy provider rolls within a few cycles, a healthy one never does.
	for i := 0; i < 3; i++ {
		s.restartProvider()
